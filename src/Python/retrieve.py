...
class _paths(Enum):
    ...
    STOCK = '../storage/data/lateststock.txt'
    METADATA = '../storage/data/metadata.txt'
    STOCKGRAPH = '../storage/stockgraph.png'


def plot_graph(ys, path_to_graph):
    # rcParams['figure.figsize'] = 40, 12
    num_ys = len(ys)
    xs = np.linspace(0, num_ys, num_ys)
    plt.plot(xs, ys, c='black')
    # plt.show()
    plt.savefig(path_to_graph, transparent=True, dpi=200)
...
def retrieve(data_category: str, **kwargs) -> dict:
    ...
    **kwargs
    ...
    match (data_category.lower()):
        case "stock":
            stock_data: dict = get_stock_data(**kwargs)
            data_keys = list(stock_data.keys())
            if data_keys[0] != "Meta Data":
                pu.write_to_path(_paths.STOCK.value, "NONE")
                pu.write_to_path(_paths.METADATA.value, "NONE")
                raise ValueError("Invalid Stock Symbol")
            
            md = ""
            metadata_dict: dict = stock_data["Meta Data"]
            meta_keys = list(metadata_dict.keys())

            for key in meta_keys:
                md += key + ": " + metadata_dict[key] + "\n"
            pu.write_to_path(_paths.METADATA.value, md)

            stock_value_data: dict = stock_data[data_keys[1]] # stock values
            sv_keys = list(stock_value_data.keys()) # stock dates
            
            latest: dict = stock_value_data[sv_keys[0]] # latest date for value data
            latest_keys = list(latest.keys()) # open, close, high, low, etc.
            stock_l_s = sv_keys[0] + "\n"
            for lkey in latest_keys:
                stock_l_s += lkey + ": " + latest[lkey] + "\n"
            stock_l_s += "\n"
            pu.write_to_path(_paths.STOCK.value, stock_l_s)

            # below is the stock graph
            
            close_key = '4. close'
            xs: list[int] = []
            for sv_key in sv_keys:
                stock_value_dict_at_sv_key: dict = stock_value_data[sv_key]
                xs.append(float(stock_value_dict_at_sv_key[close_key]))
            # print(xs)
            plot_graph(xs, _paths.STOCKGRAPH.value)
